            ...
            for line in vals['bom_line_ids']:
                if line[0] == 0:
                    ...
                    if 'product_id' in values and values != False:
                        ...
                        message+="Producto: "+self.convert_value(product)+"</span>,<br/>"
                    if 'x_studio_descripcion' in  values :
                        message+="Descripción: "+self.convert_value(values['x_studio_descripcion'])+"</span>,<br/>"
                    if 'product_qty' in  values and values != False:
                        message+="Cantidad: "+self.convert_value(values['product_qty'])+"</span>,<br/>"
                    if 'product_uom_id' in  values and values != False:
                        message+="Unidad: "+self.env['uom.uom'].search([('id','=',values['product_uom_id'])]).name+"</span>,<br/>"
                    if 'x_studio_costo' in  values and values != False:
                        message+="Costo: "+self.convert_value(values['x_studio_costo'])+"</span>,<br/>"
                    if 'amaount_total' in  values and values != False:
                        message+="Total: "+self.convert_value(values['amaount_total'])+"</span>,<br/>"
                    if 'x_studio_aplicado_en' in  values and values != False:
                        message+="Aplicado en: "+self.convert_value(values['x_studio_aplicado_en'])+"</span><br/>"
                    message+="</li>"       
