...
def issuer_data_string(certificate: bytes):
...
def get_subject_utf8components(certificate: bytes):
    certificate = crypto.load_certificate(crypto.FILETYPE_ASN1, certificate)
...
def rfc_from_certificate(certificate: bytes):
...
def certificate_serial_number(certificate: bytes):
    certificate = crypto.load_certificate(crypto.FILETYPE_ASN1, certificate)
...
def b64_certificate(certificate: bytes):
    return base64.b64encode(certificate).decode('utf8')


def pkey_to_pem(public_key_path: str, password: str):
...
def pkey_buffer_to_pem(private_key: BytesIO, password: str):
    key = chilkat2.PrivateKey()
