...
class CFDIAccountDashboardManager(models.Model):
    ...
    company_id = fields.Many2one('res.company', string='Compañia', change_default=True, required=True, readonly=True, 
                                    default=lambda self: self.env['res.company']._company_default_get('cfdi.account.dashboard.manager'))
    ...
    def action_download(self):
        ...
        for rec in self:
            ...
            if rec.download_pending == False and rec.file:
                return {
                        'type': 'ir.actions.act_url',
                        'url': file_url,
                        'target': 'new'
    ...
    def _run_download_pending_package_from_sat(self):
        ...
        for consulta in records_pending:
            ...
            if difference_between_request >= time_to_download:
                ...
                company_user = consulta.company_id
                if not company_user:
                    company_user = consulta.user_id.company_id

                if company_user.download_automatically:
                    ...
                    _logger.info("\n####################### La Compañia %s no esta autorizada para descargar automaticamente los paquetes del SAT. Consulta ID %s " % (company_user.name,consulta.id))
        # for move in records:
        #     date = None
...
class AccountCFDIMultiDownload(models.TransientModel):
    ...
    def b64str_to_tempfile(self, b64_str=None, file_suffix=None, file_prefix=None, context=None):
        """
        ...
        #f.write(b64_str )
        f.write(base64.decodestring(b64_str or b''))
        f.close()
        os.close(fileno)
        return fname
    ...
    def _get_file_globals_download(self):
        ...
        accouunt_data = self.env['l10n_mx_edi.certificate'].search([])[-1]
        prem_key = accouunt_data.get_pem_key(accouunt_data.key, accouunt_data.password)
        cer_pem, certificate = self.env['l10n_mx_edi.certificate'].search([])[-1].get_data()

        accouunt_data = self.env['res.config.settings'].search([])[-1]

        #certificate_file, cf_pth = self.b64_str_to_physical_file(accouunt_data.content, 'cer', 'cer')
        #certificate_key_file, kf_pth = self.b64_str_to_physical_file(accouunt_data.key, 'key', 'key')
        #certificate_password = accouunt_data.password

        certificate_file, cf_pth = self.b64_str_to_physical_file(accouunt_data.certificate_file, 'cer', 'cer')
        certificate_key_file, kf_pth = self.b64_str_to_physical_file(accouunt_data.certificate_key_file, 'key', 'key')
        certificate_password = accouunt_data.certificate_password
        
        _logger.info(certificate_file)
        _logger.info(certificate_key_file)
    ...
    def get_download_check_request(self, file_globals, id_solicitud):
        ...
        result = {}
        _logger.info("\n################ RFC VALIDATOR %s  " % rfc_solicitante)
    ...
    def execute_download(self):
        for rec in self:
            ...
            if no_data == False and download_pending == False:
...
# FIEL_KEY = '/tmp/estrasol.key'
# FIEL_CER = '/tmp/estrasol.cer'
# FIEL_PAS = 'prueba'
